<template>
    <div id="messages">
        <div v-if= "isAuth">
            <div  class="my-3 h-80 d-inline-block ml-lg-5" style="width: 95%">

                <div v-for="message in messages" :key="message.id" >

                    <div class="my-3 tape" style="width: 42%">
                        <b-card :title="message.login" >
    ...
    export default {
        ...
        methods: {
            getMessages: function() {
                if(this.mySubscribe === 'mySubscribe') {
                    api.getMessagesFromSubscribe(this.userId, {headers: this.header})
                        .then(result => {
                            this.messages = [];
                            result.data.forEach(message => this.messages.push(message));
                        });
                } else {
                    api.getAllMessages({headers: this.header})
                        .then(result => {
                            this.messages = [];
                            result.data.forEach(message => this.messages.push(message))
                        });
